...
**Demo user:** Priya Sharma (`demo@smartspend.ai` / `demo1234`) — an at-risk user who is overspending. All demo interactions should tell her story.
...
- `smartspend-ai/ai-service/seed_data.json` — 600 synthetic transactions across two users (Priya = at-risk demo user, Alex = healthy user)
...
**Goal:** A fully working, visually polished React application with login, dashboard, and transaction pages. At the end of this phase the demo user Priya can log in and see her health score, risk card, forecast chart, category breakdown, and transaction feed — all with real data.
...
**`RiskCard`** — shows risk level badge (low/medium/high with color), overspend amount if any ("On track to overspend by ₹6,840 this month"), top 2-3 risk factors as bullet points, and a projected end-of-month balance. Use red background tint for high risk. This card must feel urgent for the demo user Priya.
...
- Dashboard shows Priya's real health score (should be in the 40-65 range)
- ForecastChart renders 30 data points from the insight
- CategoryBreakdown shows at least 5 categories with real amounts
- RiskCard shows risk_level "high" or "medium" for Priya with at least 2 risk factors
...
For the demo, after login and refresh, Priya should have at least 3 alerts generated: one overspend pace alert, one or two category breach alerts. These come from the `buildAlerts` function in the backend insights route.
...
- AlertsPage shows at least 3 real alerts for Priya with correct severity colors
...
Before the demo, verify these specific data points for Priya's account are realistic and impressive:
...
**Screen 1 — Dashboard:** Point to health score first. "63 out of 100. Priya is financially stressed but doesn't know it yet. She's going to overspend by ₹6,800 this month."
...
**Screen 3 — Forecast chart:** "This isn't history. This is the future. Without intervention, Priya hits near-zero by the 28th."

**Screen 4 — Chat (the wow moment):** Type "What would happen if I cut my dining budget by 30%?" Live tool calls, live data, specific ₹ amounts in the response. "The AI just ran a real simulation on Priya's actual data."
...
_End of SmartSpend AI Master Implementation Plan_
_Total estimated time: 5.5–6 hours of implementation + 0.5 hours demo rehearsal = 6–6.5 hours of the 8-hour window_
_Buffer: 1.5–2 hours for debugging, unexpected issues, and deployment_
